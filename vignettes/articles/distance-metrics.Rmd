...
```{=html}
<img src="../reference/figures/distance-metrics-diagram.svg" alt="Distance Metrics and Semimetrics" style="width:100%; max-width:720px; display:block; margin:1em auto;" />
```
...
Beyond elastic distance, **shape distance** factors out additional nuisance
transformations (vertical translation, scaling) on top of reparameterization.
This is useful when curves represent geometric shapes where absolute position
and scale are irrelevant.
...
d_elastic <- elastic.distance(f1, f2, argvals)
...
The shape distance is smaller because it factors out the scale and translation
differences, comparing only the intrinsic shape. Use `shape.distance.matrix()`
for all-pairs computation:

```{r shape-matrix, eval = FALSE}
dist_mat <- shape.distance.matrix(fd, quotient = "scale")
```

For a full tutorial on shape analysis, see
`vignette("articles/shape-analysis")`.
