...
conformal prediction sets without re-fitting. This is useful when the model
is expensive to train or when you want to conformalize an existing model:

```{r generic-conformal}
# Binary classification for generic conformal (requires logistic model)
train_binary <- fd_train[y_train %in% c(1, 2), ]
test_binary <- fd_test[y_test %in% c(1, 2), ]
y_train_bin <- y_train[y_train %in% c(1, 2)]
y_test_bin <- y_test[y_test %in% c(1, 2)]

# Fit logistic model
log_model <- functional.logistic(train_binary, y_train_bin, ncomp = 5)
...
| Generic | `conformal.generic.classification()` | Logistic (pre-fitted) | $\geq 1 - \alpha$ | From existing model |
...
- **Generic** when you already have a fitted logistic model and want to add
  uncertainty quantification without re-training.
