---
title: "Introduction to Smoothing Functional Data"
...
Real-world functional data almost always contains measurement noise. Raw observations
are discrete samples of an underlying smooth process, contaminated by:
...
Smoothing transforms noisy discrete observations into smooth functional objects,
revealing the true underlying signal. This is essential for:
...
```{=html}
<img src="../reference/figures/smoothing-diagram.svg" alt="Bias-Variance Tradeoff in Smoothing Diagram" style="width:100%; max-width:700px;">
```
...
|--------|----------|----------|---------------|----------|
| **P-spline** | `pspline()` | Penalized B-spline fit with automatic GCV | $\lambda$ (penalty) | General purpose, works out of the box |
| **Nadaraya-Watson** | `S.NW()` | Kernel-weighted local average | $h$ (bandwidth) | Simple nonparametric smoothing |
| **Local Linear** | `S.LLR()` | Local linear regression with kernel weights | $h$ (bandwidth) | Better boundary behavior than NW |
| **Basis Expansion** | `fdata2basis()` | Project onto B-spline or Fourier basis | $K$ (number of basis) | Dimensionality reduction + smoothing |
| **Penalized Basis (fixed)** | `smooth.basis.fd()` | Many basis functions + roughness penalty | $\lambda$ (penalty) | When you want explicit control |
| **Penalized Basis (auto)** | `smooth.basis.gcv()` | Penalized basis with GCV/AIC/BIC selection | data-driven $\lambda$ | Automatic tuning, multiple criteria |
...
The Berkeley Growth Study tracked the heights of 54 girls and 39 boys from ages 1 to 18.
This classic dataset is available in the `fda` package.
...
The data appears relatively smooth because height was carefully measured. Let's add
some realistic measurement noise to demonstrate smoothing techniques:
...
```{=html}
<img src="../reference/figures/smoothing-pspline-diagram.svg" alt="P-spline Smoothing" style="width:100%; max-width:700px; display:block; margin:1em auto;" />
```

Penalized splines (P-splines) are a powerful and flexible smoothing method. They balance
two objectives:

1. **Fit the data closely** (minimize residuals)
2. **Keep the curve smooth** (penalize roughness)

The smoothing parameter $\lambda$ controls this trade-off:
- Small $\lambda$: Follows data closely, may be wiggly
- Large $\lambda$: Very smooth, may miss real features
...
```{=html}
<img src="../reference/figures/smoothing-kernel-diagram.svg" alt="Kernel Smoothers" style="width:100%; max-width:700px; display:block; margin:1em auto;" />
```

Kernel smoothers estimate the value at each point using a weighted average of
nearby observations. The bandwidth $h$ controls the neighborhood size.
...
The default bandwidth from `h.default()` is based on the data:
...
```{=html}
<img src="../reference/figures/smoothing-basis-diagram.svg" alt="Basis Expansion" style="width:100%; max-width:700px; display:block; margin:1em auto;" />
```

Represent curves as linear combinations of basis functions. This provides
dimensionality reduction along with smoothing.
...
```{=html}
<img src="../reference/figures/smoothing-penalized-diagram.svg" alt="Penalized Basis Smoothing" style="width:100%; max-width:700px; display:block; margin:1em auto;" />
```

Penalized basis smoothing combines basis expansion (like Method 3) with a roughness
penalty. Instead of controlling smoothness by choosing the number of basis functions,
use many basis functions but add a penalty term that discourages roughness:

$$\text{minimize} \quad \|y - X c\|^2 + \lambda\, c^T D^T D\, c$$

where $D$ is a difference matrix encoding roughness.

### Fixed Lambda with smooth.basis.fd()

When you know or want to control the smoothing parameter $\lambda$ directly:
...
GCV automatically selects the optimal smoothing parameter without manual tuning:

$$\text{GCV}(\lambda) = \frac{\text{RSS}/n}{(1 - \text{edf}/n)^2}$$
...
|--------|----------|----------|---------------|
| **P-splines** | `pspline()` | General purpose, automatic tuning | $\lambda$ (smoothing) |
| **Kernel smoothers** | `S.NW()`, `S.LLR()` | Non-parametric, local adaptation | $h$ (bandwidth) |
| **Basis expansion** | `fdata2basis()` | Dimensionality reduction | $K$ (number of basis) |
| **Penalized basis (fixed)** | `smooth.basis.fd()` | Known smoothness, explicit control | $\lambda$ (penalty) |
| **Penalized basis (auto)** | `smooth.basis.gcv()` | Automatic tuning, GCV/AIC/BIC | data-driven $\lambda$ |

**Recommendations:**

1. **Start with `smooth.basis.gcv()`** — automatic $\lambda$ selection via GCV works well in most cases
2. **Use `pspline()`** for fast P-spline smoothing with GCV
3. **Use basis expansion** (`fdata2basis()`) when dimensionality reduction is the goal
4. **Use kernel smoothers** for highly irregular sampling or adaptive smoothing
5. **Use Fourier basis** (`smooth.basis.gcv(..., type = "fourier")`) for periodic/seasonal data

## See Also

- `vignette("basis-representation", package = "fdars")` — basis function representations (B-spline, Fourier)
