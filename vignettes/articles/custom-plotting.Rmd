---
title: "Custom Plotting with ggplot2"
output: rmarkdown::html_vignette
---

```{r, include = FALSE}
has_suggested <- requireNamespace("dplyr", quietly = TRUE) &&
...
## Introduction
The **fdars** package provides convenient `plot()` and `autoplot()` methods for
functional data objects. However, for publication-quality figures or specialized
visualizations, you may need more control. This vignette shows how to create
fully customizable plots using ggplot2 directly.

```{=html}
<img src="../reference/figures/custom-plotting-diagram.svg" alt="Custom Plotting with ggplot2" style="width:100%; max-width:720px; display:block; margin:1em auto;" />
```

```{r check-deps, eval=!has_suggested, echo=FALSE, results='asis'}
cat("
> **Note:** This vignette requires the `dplyr` and `tidyr` packages.
> Install them with: `install.packages(c('dplyr', 'tidyr'))`
")
```
...
- `data`: Matrix of size `[n, m]` where n is the number of curves and m is the
  number of evaluation points
- `argvals`: Vector of length m with the evaluation points (time/domain values)
...
The key to custom plotting is converting the wide matrix format to a long
(tidy) format that ggplot2 expects.
...
ggplot(df_long, aes(x = t, y = value, group = curve_id)) +
  ...
  theme_minimal()
```
### Facet by Binned Continuous Variable
...
```{r small-multiples, fig.height=8}
...
```{r publication-theme, fig.width=6, fig.height=4}
...
1. **Convert to long format**: Use the `fdata_to_long()` function shown above
...
6. **Export**: Use `ggsave()` with appropriate format and resolution
