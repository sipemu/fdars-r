...
```{=html}
<img src="../reference/figures/derivatives-diagram.svg" alt="Working with Derivatives" style="width:100%; max-width:720px; display:block; margin:1em auto;" />
```
...
```{=html}
<img src="../reference/figures/derivatives-diagram.svg" alt="Noise Amplification with Differentiation Diagram" style="width:100%; max-width:700px;">
```
...
The `deriv()` function uses central differences, which are $O(h^2)$ accurate.
For higher accuracy, `fdata.gradient()` uses a 5-point stencil on uniform
grids — achieving $O(h^4)$ accuracy — and 3-point Lagrange interpolation on
non-uniform grids.
...
|----------|-------|------|----------|
| `deriv()` | $O(h^2)$ | Any | General use, higher-order derivatives |
| `fdata.gradient()` | $O(h^4)$ uniform, $O(h^2)$ non-uniform | Any | First derivatives requiring maximum accuracy |

Use `fdata.gradient()` when accuracy of the first derivative matters (e.g.,
computing growth velocity for clinical thresholds). Use `deriv()` when you
need second or higher derivatives, or when $O(h^2)$ accuracy is sufficient.
...
| First derivative (high accuracy) | `fdata.gradient(fd)` | $O(h^4)$ 5-point stencil |
...
- `vignette("distance-metrics", package = "fdars")` — distance and semimetric measures
