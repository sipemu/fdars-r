...
At first glance, the joint model appears to perform **worse** than
either single-predictor model. However, this is a **regularization
artifact**: the same $\lambda = 1$ is applied to all three models, but
the joint model has two coefficient functions to estimate, making it
more sensitive to the penalty strength.
...
R-squared across lambda values
...
![](example-canadian-precipitation_files/figure-html/lambda-sensitivity-plot-1.png)
...
![](example-canadian-precipitation_files/figure-html/station-scatter-1.png)

The practical takeaway: when comparing FOSR models with different
numbers of predictors, either (a) select $\lambda$ separately for each
model via cross-validation, or (b) compare models at the same,
well-chosen $\lambda$.
...
- **Lambda sensitivity**: the joint model’s apparent underperformance is
  a regularization artifact. At low $\lambda$, latitude + longitude
  together outperform either alone. Always select $\lambda$ per model
  specification.
