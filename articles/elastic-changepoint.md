...
**Changepoint detection** identifies the index at which a
chronologically ordered sequence of functional observations undergoes a
structural change. The
[`elastic.changepoint()`](https://sipemu.github.io/fdars-r/reference/elastic.changepoint.md)
function provides three approaches, each sensitive to a different type
of change:
...
![Elastic Changepoint
Detection](../reference/figures/elastic-changepoint-diagram.svg)
...
- **Intra-curve time** $t \in \mathcal{T}$: the domain over which each
  ...
  example, $t$ might represent seconds within a single production cycle
  or hours within a single day.
- **Inter-curve index** $i = 1,\ldots,n$: the chronological ordering of
  curves (the row index in the fdata object). For example, $i$ might
  index successive production runs, calendar days, or patient visits.

The changepoint $\tau$ is detected on the **inter-curve axis**: “at
which curve index does the distribution of curves change?” The
intra-curve domain $t$ is used internally to compute distances, align
SRVFs, and extract FPCA features.

**Important**: The rows of the fdata object must be in chronological
order. Shuffling the rows would destroy the sequential structure that
the CUSUM test relies on.

## Mathematical Framework

### CUSUM Test for Functional Data

Given a functional time series $\{ f_{1},\ldots,f_{n}\}$ ordered
chronologically by the inter-curve index $i$, we test $H_{0}$: no
changepoint (the distribution is stationary across $i$) against $H_{1}$:
there exists $\tau \in \{ 1,\ldots,n - 1\}$ such that the distribution
changes at $\tau$.

The **CUSUM (cumulative sum)** statistic for a scalar summary $S_{i}$
extracted from each curve is

$$T_{n}(k) = \frac{1}{\widehat{\sigma}\sqrt{n}}\left| \sum\limits_{i = 1}^{k}S_{i} - \frac{k}{n}\sum\limits_{i = 1}^{n}S_{i} \right|,\qquad k = 1,\ldots,n - 1,$$

and the test statistic is $T_{n} = \max_{k}T_{n}(k)$, with the estimated
changepoint $\widehat{\tau} = \arg\max_{k}T_{n}(k)$.

### How the Summary Statistic Differs

- **Amplitude**:
  $S_{i}^{(\text{amp})} = \parallel {\widetilde{q}}_{i} - \bar{q} \parallel_{2}$
  (distance between aligned SRVF and Karcher mean SRVF). Sensitive to
  shape changes only.
- **Phase**:
  $S_{i}^{(\text{ph})} = d_{\Gamma}\left( \gamma_{i},\text{id} \right)$
  (elastic distance between warp function and the identity). Sensitive
  to timing changes only.
- **FPCA**: Multivariate CUSUM on the FPC scores ${\mathbf{ξ}}_{i}$.
  Sensitive to changes in either amplitude, phase, or both.

### Permutation Inference

Under $H_{0}$, the ordering of curves is exchangeable. The p-value is
estimated by randomly permuting the sequence $B$ times:

$$\widehat{p} = \frac{1}{B}\sum\limits_{b = 1}^{B}\mathbf{1}\{ T_{n}^{(b)} \geq T_{n}\}.$$

## Example 1: Pure Amplitude Changepoint

In this scenario, the **shape** of the curves changes at $\tau = 30$ but
the timing stays the same. Before the break: simple sine waves. After:
the amplitude doubles and a second harmonic appears.

![Amplitude Changepoint: shape changes, timing stays the
same](../reference/figures/changepoint-amplitude-diagram.svg)
...
![](elastic-changepoint_files/figure-html/plot-amp-data-1.png)
...
larger CUSUM, confirming the change is in shape, not timing. The phase
test may also reach significance because shape changes can induce
spurious phase variation, but the CUSUM ratio clearly indicates a
shape-driven change.
...
![](elastic-changepoint_files/figure-html/plot-amp-cusum-1.png)

``` r
plot(res_amp_a, type = "data")
```

![](elastic-changepoint_files/figure-html/plot-amp-segments-1.png)

## Example 2: Pure Phase Changepoint

Here the **shape** is identical before and after, but the **timing**
shifts: after $\tau = 30$, the same template is observed through a
nonlinear time warp (as if the process runs faster at the start and
slower at the end).

![Phase Changepoint: timing changes, shape stays the
same](../reference/figures/changepoint-phase-diagram.svg)
...
![](elastic-changepoint_files/figure-html/plot-ph-data-1.png)
...
![](elastic-changepoint_files/figure-html/plot-ph-cusum-1.png)

``` r
plot(res_ph_p, type = "data")
```

![](elastic-changepoint_files/figure-html/plot-ph-segments-1.png)
...
![Mixed Changepoint: both shape and timing
change](../reference/figures/changepoint-mixed-diagram.svg)
...
![](elastic-changepoint_files/figure-html/plot-mix-data-1.png)

All three tests detect the changepoint, but the FPCA-based test — which
captures both amplitude and phase variation — produces the strongest
signal:
...
![](elastic-changepoint_files/figure-html/plot-mix-cusum-1.png)

``` r
plot(res_mix_f, type = "data")
```

![](elastic-changepoint_files/figure-html/plot-mix-segments-1.png)
...
After detecting a changepoint at $\widehat{\tau}$, we can quantify *how
large* the change is by comparing the two segments. Two natural effect
size measures are:

- **L2 effect size**: the root-mean-square distance between the mean
  curves of the before and after segments,
  $$\Delta_{\text{L2}} = \left( \int_{\mathcal{T}}({\bar{f}}_{\text{before}}(t) - {\bar{f}}_{\text{after}}(t))^{2}\, dt \right)^{1/2}.$$
- **Relative change**:
  $\Delta_{\text{L2}}/ \parallel {\bar{f}}_{\text{before}} \parallel_{2} \times 100\%$,
  expressing the shift as a percentage of the baseline signal magnitude.
...
The **CUSUM statistic itself** is also a measure of change strength: it
scales monotonically with the signal-to-noise ratio. Comparing CUSUM
values across the amplitude and phase tests reveals the *nature* of the
change (shape vs timing), while the magnitude indicates its severity.

## Detection Sensitivity and the Role of Noise

A key practical question is: **how strong must a break be for the test
to detect it?** The answer depends not just on the signal strength but
on the **noise level**. The same 50% amplitude increase is easy to
detect at $\sigma = 0.05$ but may be invisible at $\sigma = 0.4$. We
therefore sweep both dimensions to produce **detection surfaces**.

### Amplitude × Noise Surface

We vary the amplitude scaling factor and the observation noise
simultaneously.
...
The heatmap shows the full detection surface, with the $p = 0.05$
contour marking the detection frontier:
...
![](elastic-changepoint_files/figure-html/amp-heatmap-1.png)
...
![](elastic-changepoint_files/figure-html/phase-heatmap-1.png)

### CUSUM Degradation with Noise

Fixing the amplitude factor at 1.5, we see how the CUSUM statistic
degrades as noise increases. The test statistic shrinks because the
signal is increasingly buried in noise.
...
![](elastic-changepoint_files/figure-html/cusum-vs-noise-1.png)

### Minimum Detectable Effect

For each noise level, the smallest amplitude factor and warp strength
where the test is significant:
...
- **High noise** ($\sigma \geq 0.4$): only dramatic changes (2x+
  amplitude) are reliably detected.
- **Phase detection is inherently more robust**: elastic alignment
  removes amplitude variation before testing, so the phase test is less
  affected by amplitude noise. This makes it the preferred first test
  when the noise level is uncertain.

## Process Monitoring: Statistical Decision Boundary

For process monitoring applications, a binary **OK / Not OK** decision
requires a statistically valid threshold. The permutation p-value
provides exactly this: at significance level $\alpha$, reject $H_{0}$
(no change) when $p \leq \alpha$.
...
2.  **Run
    [`elastic.changepoint()`](https://sipemu.github.io/fdars-r/reference/elastic.changepoint.md)**
    with `n.mc >= 1000` for a stable p-value.
3.  **Decision**:
    - If $p \leq \alpha$ (e.g., $\alpha = 0.05$): **reject** $H_{0}$. A
      structural change is detected at the estimated $\widehat{\tau}$.
    - If $p > \alpha$: **do not reject**. No significant change
      detected.
...
- **Multiple testing**: If you test the same data with amplitude, phase,
  and FPCA tests, apply a Bonferroni correction ($\alpha/3$) or use a
  single FPCA test as a screening test.
- **Sample size**: The permutation test has a minimum achievable p-value
  of $1/(B + 1)$, where $B$ = `n.mc`. Use `n.mc >= 999` for decisions at
  $\alpha = 0.05$, and `n.mc >= 9999` for $\alpha = 0.01$.
- **Prospective monitoring**: For ongoing monitoring of a production
  process, apply the test to a sliding window of the most recent $n$
  curves. This provides near-real-time detection of process shifts.

## Applications

Elastic changepoint detection is useful whenever a sequence of
functional observations may undergo a structural break. The separation
of amplitude and phase is the key advantage over classical changepoint
methods.
...
- **Amplitude changepoint**: detects when the *shape* of production
  curves changes (defective product, drift in process output).
- **Phase changepoint**: detects when the *timing* of a process step
  shifts (e.g., a heating ramp slows down) even if the final product
  still meets specs.

### Biomedicine and Clinical Trials

Longitudinal patient curves (EEG signals, growth trajectories, gait
cycles) may change during treatment or disease progression.

- Detect when a patient’s gait pattern changes after surgery (amplitude)
  or when stride timing shifts (phase).
- In clinical trials, identify the time point at which a drug begins to
  take effect by monitoring functional biomarker profiles.
...
- **Amplitude changepoint**: identifies when seasonal patterns intensify
  or weaken (e.g., hotter summers after a climate shift).
- **Phase changepoint**: identifies when seasons shift in timing (e.g.,
  earlier spring onset due to warming trends).

### Speech and Audio Processing

Speaker intonation profiles or phoneme spectrograms may change
mid-recording due to emotion, fatigue, or speaker switches.
...
daily may undergo structural breaks during policy changes or market
crises.
...
**Use phase changepoints** for changes in timing (speech patterns,
seasonal shifts).
...
- Aue, A. and Horv0e1th, L. (2013). Structural breaks in time series.
  *Journal of Time Series Analysis*, 34(1), 1–16.

- Berkes, I., Gabrys, R., Horv0e1th, L. and Kokoszka, P. (2009).
  Detecting changes in the mean of functional observations. *Journal of
  the Royal Statistical Society: Series B*, 71(5), 927–946.
